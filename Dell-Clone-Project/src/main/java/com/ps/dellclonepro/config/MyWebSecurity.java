...
public class MyWebSecurity {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().requestMatchers("/", "/403").hasAnyAuthority("USER", "ADMIN")
				.requestMatchers("/deleteProduct/**","/addproducts","/updateProductForm/**").hasAuthority("ADMIN").anyRequest()
				.authenticated().and().formLogin().loginProcessingUrl("/login").successForwardUrl("/")
